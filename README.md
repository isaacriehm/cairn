...
- **Curates `.harness/ground/`** — every binding architectural decision,
...
- **Exposes ground state via an MCP server** — agents query
  `harness_decisions_in_scope`, `harness_invariants_in_scope`,
  `harness_canonical_for_topic`, etc. instead of guessing or fabricating.
- **Visible adoption** — `/harness-init` (or auto-detected by the plugin)
...
- **Multi-developer enforcement** — versioned git hooks, per-clone bootstrap
  (`harness join`), CI gate, and Claude Code degraded mode mean every
  contributor runs the harness or commits don't merge.

## Key concepts

| Concept | What |
|---------|------|
| **Decision (DEC-NNNN)** | A binding architectural choice with rationale, scope globs, supersedes chain. Stored at `.harness/ground/decisions/<id>.md`. |
| **Invariant (§V&lt;N&gt;)** | A domain rule whose violation is a bug, not a style preference. Stored at `.harness/ground/invariants/<id>.md`. |
...
For a new contributor cloning an already-adopted project, `harness join` is
...
Source tree pre-public-rename uses the historical `harness*` package
names; the search-replace to `cairn*` lands as part of the public-repo
initial commit. See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) §3 for
the layer-by-layer breakdown.

## Multi-developer enforcement

Once a project is Cairn-adopted, every developer who touches it runs the
harness — locally and at PR time. Four layers per
[`docs/PLUGIN_ARCHITECTURE.md`](docs/PLUGIN_ARCHITECTURE.md) §17:

| Layer | Where | Catches |
|-------|-------|---------|
| 1 — versioned git hooks | `.harness/git-hooks/{pre,post,commit-msg}-commit` | Local commit attempts that violate ground state. |
| 2 — `harness join` bootstrap | CLI + `package.json prepare` | New clones that haven't activated `core.hooksPath`. |
| 3 — CI gate | `.github/workflows/harness-check.yml` | `git commit --no-verify` bypass slipping through. Non-bypassable. |
...
```
.harness/
├── config.yaml                   slug, harness_version, project_globs, sensors
...
| [`docs/FILESYSTEM_LAYOUT.md`](docs/FILESYSTEM_LAYOUT.md) | `.harness/` directory contract |
...
The plugin architecture takes cues from OpenAI's harness lesson on agent
