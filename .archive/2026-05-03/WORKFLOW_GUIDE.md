...
| **3** | Opus 4.7 | High-stakes implementer (sample-project-specific gnarly modules), escalation when Tier 2 fails twice, operator-explicit `--tier opus` | Two consecutive fails → page operator |

### 2.1 Per-task-class assignment (`.harness/config/workflow.md` sample-project: block)
...
`npx @isaacriehm/harness init` checks `ollama list`. If missing or required models absent, prompts:
...
**The harness package code is project-agnostic.** It reads the project-specific block by `Object.keys()` lookup at runtime — never by hardcoded project name (per operator answer S1: "the harness should propose sensors, agnostically, like dont mention 'sample-project.' ANYWHERE within harness code, only internal docs"). The block key matches the adopting project's `package.json name` field (or directory name as fallback). Below shows the sample-project-adopted shape; for any other project, replace `sample-project:` with that project's name.
...
sample-project:
  ...
  off_limits:
    ...
    - core/REVIEW_DECISIONS.md  # sample-project-historical, leave alone
    - docs/decisions/
    - docs/design/brand/
  high_stakes_globs:
    - core/src/calls/**
...
The harness pkg has zero hardcoded `"sample-project"` strings in source. Adopting a different project = change `sample-project:` to `acme:` (or whatever the project name is) and update the values; harness reads by key lookup.
