...
# Residual Questions — Cairn Project
...
```
P1: default  P2: @isaacriehm/harness
...
D1: 1487133145013944443 (sample-project. discord)  D2: 1264005138918408204  D3: default  D4: default
N1: ___  N2: ___  N3: ___  N4: ___
S1: the harness should propose sensors, agnostically, like dont mention "sample-project." ANYWHERE within harness code, only internal docs. we should ask the user  S2: default
...
| #   | Decision                                                | Confirm                                                   |
| --- | ------------------------------------------------------- | --------------------------------------------------------- |
| P1  | Project name `Cairn` (capitalized; folder `Cairn/`) | default                                                   |
...
| --- | --------------------------------------------- | -------------------------------------------------------------------------------------------- | ------ |
| A1  | First adopted project for the trial run       | sample-project (`<operator-home>/sample-project/`)                          |        |
| A2  | Pilot module within sample-project for first agent run | `core/src/integrations/`                                                                     |        |
| A3  | Off-limits paths (sample-project-specific extension)   | `core/RESUME_PROMPT.md`, `core/REVIEW_DECISIONS.md`, `docs/decisions/`, `docs/design/brand/` |        |
...
| D3  | Bot identity name                                                              | `sample-project-harness` |        |
...
| #   | Question                                                                                                             | Default                                                                      | Answer |
| --- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ------ |
| S1  | Should the harness propose sample-project-specific sensors automatically at init, or wait for explicit operator confirmation? | propose, await operator 🟢 per sensor                                        |        |
| S2  | Initial sensor disable list (sensors that have a known false-positive rate >50% on sample-project today)                      | none — all sensors enabled at adoption; disable per-failure via `/oops` flow |        |
...
| #   | Question                                              | Default                                                                                                   | Answer |
| --- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------ |
| M1  | Where does the harness pkg's source live              | inside this `Cairn/` repo as a `harness/` workspace package; published to npm at `@isaac/harness` later |        |
