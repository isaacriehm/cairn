...
You are completing the Cairn codebase — a TypeScript pnpm monorepo that acts as a "project brain" for Claude Code. It maintains ground state (decisions, invariants, brand, product context), hooks into Claude Code's session lifecycle to inject that state, and runs deterministic sensors on diffs before they land.
...
- 2-sentence description of Cairn
...
- `import { readStatusForCLI } from "@isaacriehm/harness-core"` resolves cleanly.
...
- `import { buildHandoffBlock, buildSpecDelta, type SpecDelta } from "@isaacriehm/harness-core"` resolves cleanly.
...
- `import { runReadEnricher } from "@isaacriehm/harness-core"` resolves cleanly.
...
- `packages/harness-core/src/init/seed.ts` (FULL FILE — to confirm the template is copied verbatim by `seedCairnLayout`)
- `docs/READ_ENRICHER_SPEC.md` §2 (hook registration)
- `docs/PRIMER.md` §8.6 (hook priority order)

**Context to paste:** Full content of all four files.

**IMPORTANT — earlier draft of this task was wrong.** It instructed editing `init.ts` to write hooks programmatically. In reality, `init.ts` does NOT write `.claude/settings.json` directly. The file is shipped as a **template** at `packages/harness-core/templates/.claude/settings.json` and copied verbatim by `seedCairnLayout` (see `src/init/seed.ts`). The fix is to edit the template; `seed.ts` handles the rest.
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx -y @isaacriehm/harness hook session-start"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx -y @isaacriehm/harness hook read-enrich"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx -y @isaacriehm/harness hook write-guard"
          }
        ]
      },
      {
...
Note: the `command` strings use `npx -y @isaacriehm/harness` (matching the existing SessionStart entry's pattern), NOT the bare `harness` from earlier drafts. This ensures consistency for fresh adoptions where `harness` may not yet be on PATH.
...
- The file contains exactly one `SessionStart` array entry and three `PostToolUse` entries (Read, Write, Edit), each invoking `npx -y @isaacriehm/harness hook <event>`.
...
**No code changes to `seed.ts`.** `seedCairnLayout` already walks the templates dir and copies everything; the existing `applyPlaceholders` no-ops on these new files (correct — they don't need slug substitution).

**Acceptance criteria:**
- `cd packages/harness-core && npx tsc --noEmit` passes.
- The five new files exist under `packages/harness-core/templates/.harness/ground/`.
- A test invocation of `seedCairnLayout` against a fresh temp dir produces all five files at the expected paths under `.harness/ground/`.
