 ...
 *
 * Generates `~/Library/LaunchAgents/com.isaacriehm.harness.<slug>.plist`
...
function usage(): never {
  console.error(
    "Usage: harness install <subcommand> [options]\n" +
      ...
      "  ~/Library/LaunchAgents/com.isaacriehm.harness.<slug>.plist",
  );
  process.exit(1);
}
...
export function buildLaunchdPlist(opts: PlistOptions): PlistResult {
  const label = `com.isaacriehm.harness.${opts.project}`;
  const args: string[] = [
    opts.harnessBin,
    "daemon",
    "--project",
...
async function doUninstall(flags: ParsedFlags["flags"]): Promise<void> {
  ...
  const dryRun = flags["dry-run"] === true;
  if (!dryRun) requireMacOs();
  const label = `com.isaacriehm.harness.${project}`;
  const plistPath = launchdPlistPathFor(label);
  if (dryRun) {
    process.stdout.write(
