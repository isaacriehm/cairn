/**
 * Phase 7 — fast model semantic judge wrapper.
 *
...
/**
 * Per-build counters for the fast model judge. The orchestrator
 * (`buildTopicIndex`) constructs one and threads it through
 * `makeModelJudge`; the judge increments per call so the phase
 * output can split `judge_calls` into cached vs fresh vs errored.
 * Smokes that pass a mock judge skip this surface entirely — the
 * tally fields stay at 0, which is correct (no fast model spend at all).
 */
...
// network or fast model-side latency the 20s ceiling was hitting the
...
/**
 * Return a semantic judge implementation that calls fast model.
 */
export function makeModelJudge(opts: JudgeOptions = {}): SemanticJudge {
  return async ({ a, b }): Promise<SemanticVerdict> => {
    ...
    try {
      ...
      // Surface ModelRunnerError (timeout / rate_limit / overloaded / auth) to
      // the resolver so it can trip its circuit breaker and stop calling
      // the judge instead of burning wall-time on doomed retries.
      // Non-runner errors (parse glitches, schema mismatch) are local to
      // a single pair — log + fall back to "different".
      if (err instanceof ModelRunnerError) throw err;
      log.warn({ err, a: a.file, b: b.file }, "fast model judge failed; falling back to 'different'");
      return "different";
    }
  };
}
