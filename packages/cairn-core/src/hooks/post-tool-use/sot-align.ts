...
/**
 * Layer A reuses phase 7b's walkSourceComments — essay-detection logic
 * battle-tested on real repos, with per-language comment-block /
 * JSDoc / line-cluster heuristics already calibrated.
 */
function extractBlocks(repoRoot: string, filePath: string): CommentBlock[] {
  const walk = walkSourceComments({
    repoRoot,
    onlyFiles: [filePath],
  });
  // License headers are never aligned (operator-supplied legal text,
  // never decisions / invariants).
...
function topKCandidates(
  ...
  for (const e of entries) {
    ...
    scored.push({ id: e.dec_id, similarity: score, body_hash: e.body_hash, sot_path: e.sot_path });
  }
...
function readEntityBody(repoRoot: string, id: string): string | null {
  if (repoRoot.length === 0) return null;
  const dir = id.startsWith("INV-")
    ? invariantsDir(repoRoot)
    : decisionsDir(repoRoot);
  const abs = join(dir, `${id}.md`);
