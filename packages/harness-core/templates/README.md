...
This directory ships with the `@isaacriehm/harness` npm package. It is the seed for `.harness/` and `.archive/` inside any project that runs:

```sh
npx @isaacriehm/harness init <repo-dir>
```
...
This directory is **NOT** the harness's own state. The Cairn package source repo is not self-hosted; it is the source for the published npm package.
...
Templates are the single source of seed configuration. Changes here flow to every freshly-adopted project on next `npx @isaacriehm/harness init`. Existing adoptions are unaffected — they own their copies.
