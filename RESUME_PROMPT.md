...
**Status:** Implementation in progress. Phase 16 init landed; Phases 0–14 + 14.x + 16 in tree (~20 founder-days). Phase 15 (trial-run pilot) is next — operator-driven, exercises the just-shipped init against sample-project end-to-end. **Documentation in `docs/` is still the source of truth for design; the code in `harness/` is the runtime that implements it.**
