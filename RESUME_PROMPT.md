...
**Status:** Implementation in progress. Phase 16.3 (agent ask-operator MCP tool) landed; Phases 0–14 + 14.x + 16 + 16.1/16.2/16.3 in tree (~21 founder-days). Phase 15 (trial-run pilot) is next — operator-driven, exercises the now-fully-guided init + Discord polish + agent question-asking against sample-project end-to-end. **Documentation in `docs/` is still the source of truth for design; the code in `harness/` is the runtime that implements it.**

## 1A. Implementation snapshot (binding — verify against `git log` before acting)

The Cairn repo is **NOT self-hosted**. It's the source for the published npm package `@isaacriehm/harness`. The `.harness/` shape lives in `harness/templates/`; the init script copies it into adopting projects. Do not create `.harness/` or `.archive/` at the repo root.
...
pnpm -F @isaacriehm/harness typecheck          # tsc -b --noEmit
pnpm -F @isaacriehm/harness check:layout       # validates pkg+templates + scans for banned project names
pnpm -F @isaacriehm/harness smoke:mirror       # ephemeral bare-origin + user-tree round-trip
...
pnpm -F @isaacriehm/harness smoke:tightener    # vague→blocked, clear→sonnet judgment, ship_anyway→forced; SKIPS without `claude` CLI auth
...
```
harness/
├── package.json         @isaacriehm/harness@0.0.0; deps: discord.js v14, smart-whisper,
...
- **Project-agnostic pkg code.** No `sample-project` / `Sample project` / `SAMPLE-PROJECT` strings anywhere under `harness/src/`, `harness/scripts/`, or `harness/templates/`. The `check-layout` sensor enforces this. The `<project_name>:` placeholder in `templates/.harness/config/workflow.md` is replaced by the init script with the adopting project's name.
- **`.harness/` lives in adopted projects, not in this repo.** Do not create `.harness/` or `.archive/` at the Cairn repo root. Templates ship under `harness/templates/`.
...
- **No trial-run pilot.** Phase 15 — exercise full lifecycle on a real backlog item from an adopted project (sample-project). Voice variant + decision-capture variant per spec.
...
- **better-sqlite3 native binding has the same path-with-spaces problem as smart-whisper.** node-gyp's generated Makefile doesn't quote `module_root_dir`, so projects living at `<operator-home>/project` fail to build. `setup:uat-sql --build-binding` reuses the setup-whisper /tmp-staging trick: copy the package to a no-space path, build there, copy `build/` back.
...
| `_research/STALENESS_INVENTORY.md` | ✅ existing | Sample project docs staleness audit |
| `_research/DISCORD_WHISPER_DESIGN.md` | ✅ existing | Discord+Whisper feasibility |

**Frontmatter `depends-on:` paths inside the docs may still reference `docs/orchestration/...` from the prior sample-project location.** A path-fixup pass replaces those with relative paths inside `docs/`. If the operator hits broken pointers, run that fixup.
...
| # | Decision | Rationale |
|---|----------|-----------|
| L01 | TypeScript-first stack | Operator uses Claude Code primarily; sample-project is TS-everywhere; consistency |
| L02 | pnpm monorepo workspace package (`harness/`) inside the harness project itself | Same pattern as sample-project's `core/`, `platform/`, etc. Generic via `npx @isaacriehm/harness init` for other projects |
...
| L06 | Parallel mirror checkout at `~/.local/harness/repos/<project>/` | Cairn operates here; user's working tree is sacred |
...
| L28 | No phase-gates between modules | Cairn adopts modules as operator opts in; no GSD-style ceremony |
...
| L36 | Generic harness pkg + per-project `.harness/config.yaml` | Portable; sample-project-specific config never bleeds into harness pkg |
| L37 | `npx @isaacriehm/harness init` runs deep mapper (Tier 2 LLM, one-time, OK to spend tokens) | Deep project mapping at adoption time |
...
| L49 | Distribution mechanism for early testers | Per operator answer M1. Private GitHub repo via `npm install <git-url>`; tarball via `pnpm pack`; or symlink-clone for collaborative debug. Eventual `npm publish @isaacriehm/harness` once stable. |
| L50 | Project-agnostic harness pkg code (no hardcoded project names) | Per operator answer S1: "the harness should propose sensors, agnostically, like dont mention 'sample-project.' ANYWHERE within harness code, only internal docs." Pkg reads project-specific extension block via `Object.keys()` lookup keyed on adopting project's package name. sample-project-specific text in this doc set is illustrative, not source. |
| L51 | A2 — pilot SCOPE is full sample-project repo, not just integrations module | Per operator answer A2. `pilot_module: ALL` in WORKFLOW.md `sample-project:` block. High-stakes globs trigger Layer E + cross-tenant fixture for sensitive areas. |
...
- Moved everything from `sample-project/docs/orchestration/` to `Cairn/docs/`. Cleaned up old location.
...
| Portability scope | Generic `@isaacriehm/harness` package + per-project `.harness/config.yaml`; init script does deep mapping (LLM-OK, one-time) |
...
| P1 project name `Cairn` | default |
| P2 package name | `@isaacriehm/harness` (NOT `@isaac/harness`) |
| A1 first adopted project | sample-project |
| A2 pilot scope within sample-project | **full repo** — operator quote: *"no? we are implementing harness on the full project"* |
...
| D1 Discord guild ID | `1487133145013944443` (sample-project. discord) |
| D2 owner Discord user-ID | `1264005138918408204` |
| D3 bot identity | default `sample-project-harness` |
| D4 DM enabled | default yes |
| N1-N4 Notion adapter | omitted at v0; build lazily when operator's friend wants it |
| S1 sensors | **harness pkg code MUST be project-agnostic — NO `sample-project.` strings ANYWHERE in harness code, only in internal docs** — operator quote verbatim. Cairn proposes; user approves per sensor at adoption. |
...
| M1 distribution | private GitHub repo OR tarball OR symlink for early testers; eventual `npm publish @isaacriehm/harness` |
| M2 auto-install Ollama | yes (with A/B/C dialog at init) |

## 10. What the operator wants next (most likely)

Phase 16 is complete. Next is **Phase 15 — trial-run pilot** (`docs/INTEGRATION_PLAN.md` §5 Phase 15; ~2 founder-days). The pilot exercises `harness init` against sample-project end-to-end: adopt → submit a real backlog item → watch full lifecycle → assert each acceptance criterion in §2 of the plan.

Phase 15 deliverables (per spec):

1. **Pilot task** — a real backlog item from an adopted project (sample-project): e.g. "add unique partial index on `integration_oauth_tokens(provider, user_id) WHERE archived_at IS NULL`".
...
- **Phase 16 init script** (~1.5 days) — `npx @isaacriehm/harness init` w/ stack-profile detection + E2E setup branch.
- **Phase 12.x — GC cron schedule** *(deferred to Phase 16 / adopter setup; not a Cairn-repo concern per operator decision 2026-05-02)*
...
- **Typing indicator during long phases.** `startTyping(channelId)` returns a stop fn; the orchestrator wraps tightener / implementer / reviewer / UAT calls so Discord shows "Cairn is typing…" continuously. The 8s heartbeat refresh (Discord's native indicator decays at 10s) self-stops on dead-channel detection.
...
- `now` → spawns `setup:uat-browsers` + `setup:uat-sql --build-binding` + `setup:uat-docker` directly via `runCairnSetupScript` (stdio inherited so operator sees curl/brew/pnpm progress); `e2e_setup: now` recorded in `.harness/config.yaml`
...
5. Confirm to operator in 2-3 lines, e.g.:
     "Resumed Cairn project. Phases 0–14 landed. Cheap sensors green.
...
- OpenAI, *Cairn engineering* (Feb 2026): https://openai.com/index/harness-engineering/
...
- Birgitta Böckeler, *Cairn Engineering for Coding Agent Users* (MartinFowler.com, 2026-04-02): https://martinfowler.com/articles/harness-engineering.html
- Ryan Lopopolo, *Extreme Cairn Engineering* (Latent Space, 2026): https://www.latent.space/p/harness-eng
- cavekit: https://github.com/JuliusBrussee/cavekit
- get-shit-done: https://github.com/thedotmack/get-shit-done
- claude-mem: https://github.com/thedotmack/claude-mem
- Sample project repo (proving ground): `<operator-home>/sample-project/`

---

End of resume prompt. Total context restoration target: ~98% of the previous session's working knowledge. Drop into a fresh Claude Code session at `<operator-home>/Cairn/` and proceed.
