#!/usr/bin/env bash
#
# adopt.sh — install Cairn into an adopting project in one shot.
#
# Usage:
#   ./scripts/adopt.sh <target-project-dir> [extra harness-init flags...]
#
# Example:
#   ./scripts/adopt.sh "<operator-home>/sample-project"
#   ./scripts/adopt.sh ../sample-project --skip-mirror --skip-mapper
#
...
for i in "${!PKG_DIRS[@]}"; do
  ...
  if [[ ! -f "$TGZ" ]]; then
    ...
    TGZ="$(ls -t isaacriehm-*.tgz | head -n 1)"
  fi
  ABS="$ROOT/$pkg/$TGZ"
  TARBALLS+=("$ABS")
  SPECS+=("${name}@file:${ABS}")
...
case "$PM" in
  pnpm)
    ...
    # pnpm.overrides into sample-project's package.json so pnpm uses the supplied
