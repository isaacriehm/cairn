...
export async function runInit(args: RunInitArgs = {}): Promise<InitResult> {
  ...
  if (e2eChoice === "now") {
    ...
    info(`  pnpm dlx @isaacriehm/harness setup:uat-browsers`);
    info(`  pnpm dlx @isaacriehm/harness setup:uat-sql --build-binding`);
    info(`  pnpm dlx @isaacriehm/harness setup:uat-docker`);
    info(
  ...
  info(`  cd "${repoRoot}"`);
  info(`  pnpm dlx @isaacriehm/harness watch --project ${decidedSlug}    # daemon`);
  if (detection.environment.discord_token && detection.environment.discord_guild) {
    info(
      `  pnpm dlx @isaacriehm/harness run --project ${decidedSlug} --frontend discord`,
    );
  } else {
    info(
    ...
    info(
      `  pnpm dlx @isaacriehm/harness run --project ${decidedSlug} --frontend stub`,
    );
  }
