...
1. **Cairn engineering** — the discipline of designing the controls around a model: guides that direct it, sensors that verify it, a data pipeline that grounds it. *Agent = Model + Cairn.*
...
This primer is the conceptual base. `INTEGRATION_PLAN.md` applies it to sample-project.

---

## 1. What harness engineering actually is

Birgitta Böckeler (Thoughtworks Distinguished Engineer, *Cairn Engineering for Coding Agent Users*, 2026-04-02) decomposes the harness into two mechanisms:
...
| Domain | Regulates | sample-project example |
|--------|-----------|---------------|
...
- **Cairnability is a property of the codebase.** Strongly-typed languages, definable module boundaries, conventional frameworks support more controls. sample-project is well-positioned (TypeScript + NestJS + Drizzle).
...
- Not prompt engineering. Prompt engineering tunes one inference. Cairn engineering tunes the system around many inferences over time.
...
| Pattern | Mechanism | sample-project application |
|---------|-----------|-------------------|
...
### 4.2 Layout for sample-project (filesystem-only)
...
OpenAI's reported solution (Lopopolo, multiple sources): *"treat AGENTS.md as the table of contents"*, ~100 lines, mapping to deeper progressive-disclosure docs. sample-project's current AGENTS.md is 128 lines and already TOC-shaped — keep it that way; document the rule for the generic harness package.
...
### 5.4 Single-task pipeline (sample-project default)
...
sample-project's golden-case sensor list (proposed at init, refined per `WORKFLOW_GUIDE.md` tier ladder):
...
The dishonesty problem (sample-project's 28-stub history). Six layers, stacked. Each removes a class of fakery.
...
Listed in §6 above. Project-specific from `WORKFLOW.md` `<project>:` extension block (e.g., `sample-project:`, `acme:`). The init script proposes; user accepts/rejects. **Cairn package code is project-agnostic — it reads the extension block by `Object.keys()` lookup, never by hardcoded project name.** All "sample-project-specific" references in this doc set are illustrative; in the package, `sample-project` is just the example adopted-project name.

### Layer E — Demo / E2E (high-stakes only)

For runs touching the project's high-stakes globs (per `<project>:` `high_stakes_globs` config; for sample-project: `core/src/{calls, deals, contacts, integrations, telephony}/**`): real E2E suite or recorded demo script. Catches "passed sensors but doesn't work" cases.
...
| **Confidence scores on writes** | sample-project AGENTS.md already says this | No model-issued confidence as gate or surface. |
...
| **Backward-compat shims, deprecation notices, redirects** | sample-project AGENTS.md + memory file | Hard cutovers. No transition regex. No "moved to X" stubs. |
...
OpenAI's *"recurring 'garbage collection' that scans for drift and has agents suggest fixes"* — applied to sample-project.
...
sample-project's `.claude/rules/fix-standard.md` already lists "repeat-failure items" requiring extra rigor (OAuth flow, OpenAPI drift, IA changes, form modals). Backprop turns each repeat-failure into a permanent invariant with a sensor. Repeats become detectable, then preventable.
...
| Term | Meaning |
|------|---------|
| **Cairn** | Total system around the model: guides, sensors, data context, scaffolding, evals, sandboxing, escalation. |
...
5. `INTEGRATION_PLAN.md` — phased plan against sample-project.
...
- OpenAI, *Cairn engineering: leveraging Codex in an agent-first world.* February 2026. https://openai.com/index/harness-engineering/
...
- Ryan Lopopolo, *Extreme Cairn Engineering: 1M LOC, 1B toks/day.* Latent Space, 2026. https://www.latent.space/p/harness-eng
- *OpenAI Introduces Cairn Engineering.* InfoQ, 2026-02. https://www.infoq.com/news/2026/02/openai-harness-engineering-codex/
