...
Cairn fixes all of these.

---

## 2. What Cairn is

**Cairn is a project brain.** It maintains a curated, continuously-fresh ground state for your codebase, injects that state into every Claude Code session, and deterministically evaluates everything the agent produces before it lands.
...
**Autonomy-first.** Cairn fixes things. Sensor fails → it retries with a targeted remediation prompt. Docs drift → it regenerates them. Stale content → it archives it. The operator is looped in when the harness genuinely can't decide — not as a checkpoint on every action.

Cairn is **project-agnostic**. It detects your stack at adoption time, proposes a sensor and documentation config, and asks you to confirm. No project names, frameworks, or ORMs are hardcoded in Cairn. Your project's specifics live in your `.harness/config/workflow.md` extension block.
...
Documentation management is a first-class pillar. A project accumulates docs in `docs/`, `AGENTS.md`, `.claude/rules/`, and `.harness/ground/`. Cairn owns the health of all of them.
...
`.harness/config/stub-patterns.yaml`. Cairn proposes patterns at init based on detected stack. Runs on every diff. ~5s, zero tokens. Catches: unimplemented throws, empty bodies, unsafe casts, commented-out blocks, always-undefined optionals.

### Layer B — Self-attestation cross-check

Agent emits `attestation.yaml` on completion. Cairn mechanically verifies every claim against the diff: behavior declarations, todo/stub counts, sensor claims, files touched. Lying is harder than telling the truth.
...
Registered in `.harness/config/sensors.yaml`. Proposed at init based on detected stack; confirmed per sensor. Computational only (regex, AST, structural). No LLM. Cairn package code never references project names — it reads sensor config by key.
...
**Self-repair is the default path.** When sensors fail, Cairn builds a focused remediation prompt from the exact failure (assertion ID, contradicting line, suggested fix) and re-dispatches. The operator sees none of this — it just works. Only after two failed attempts does Cairn surface to the operator, with the full structured failure context already prepared.
...
The primary delivery mechanism for ground state. Cairn is visible inside Claude Code — not hidden.

### 8.0 Status line

Cairn registers a status line in Claude Code (`.claude/settings.json` `statusLine` field). It shows at all times — present but out of the way:
...
### 8.5 Cairn Lens (VS Code / Cursor extension)

The same citation resolution logic that powers the Read enricher is exposed to humans via the **Cairn Lens** — a VS Code/Cursor extension distributed as a separate package.
...
The Lens is out of scope for the initial Cairn build — it's a separate package with its own distribution. The resolution logic in `src/hooks/post-tool-use/ledger-cache.ts` is the shared core; the Lens will consume it as a library. See `LENS_SPEC.md` (forthcoming).
...
*(For human-readable citation context in the editor, see §8.5 — Cairn Lens.)*
...
| **Hardcoded project names in Cairn code** | Cairn is project-agnostic. Project specifics live in `workflow.md` `<slug>:` block. |
...
| **Essay JSDoc blocks** | A 20-line JSDoc explaining timing attack defense, cost parameters, and a rotation script is three things in the wrong place: a decision (why we use a dummy hash), an invariant (login must call verify even for missing users), and a runbook (how to rotate). The AI should not be writing essay JSDoc — stop there. If any of that content rises to the level of a DEC or runbook, it gets captured through the normal `/direction` flow or an explicit operator ask. The code gets `// §V<N>` and nothing else. This pattern — "AI writes an essay, the essay drifts, a different run writes a different essay on the same concept" — is exactly the documentation rot problem Cairn exists to prevent. Human-readable context for citations is delivered via the Cairn Lens editor extension, not via source comments. |
...
12. **Visible, not invisible.** The operator should always know what Cairn is doing — at a glance, without asking.
...
12. `LENS_SPEC.md` — Cairn Lens VS Code/Cursor extension (forthcoming — out of scope for initial build)
