...
# Cairn — Documentation & State Management Spec
...
If any check fails: task is moved back to `tasks/active/<id>/` with `status: integrity-error`. Cairn adds an attention item. Next agent session sees the task as incomplete.
