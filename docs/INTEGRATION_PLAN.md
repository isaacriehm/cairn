...
```
                                ...
                                └────────┬─────────┘
                                         │ writes to mirror
                                ┌────────▼─────────────────────────┐
                                │  ~/.local/harness/repos/sample-project/   │
...
User's working tree at ~/Projects/sample-project/ — never touched by harness. Pulls when convenient.
```
...
**Goal:** prove end-to-end on a real task **anywhere in sample-project** (per operator answer A2 — full repo, not just integrations module). The pilot module list is the entire repo at adoption time, with `high_stakes_globs` triggering Layer E + cross-tenant fixture for the more sensitive modules.
...
**Goal:** `npx @isaacriehm/harness init <repo-dir>` lifts cleanly to any project.
...
The init script is **inquirer-driven** end-to-end (per operator instruction 2026-05-02). All operator-facing questions during `npx @isaacriehm/harness init` use `inquirer` prompts with sensible defaults pre-filled from auto-detection. The squares-into-square-holes UX rule (`docs/WORKFLOW_GUIDE.md` §1) applies: every question is `list` / `confirm` / `checkbox` first, with `input` only as the typed-default fallback. `E) Other` escape on every multi-choice question.
...
| Mechanism | Use |
|-----------|-----|
| Private GitHub repo + `npm install <git-url>` | Default. Friends granted read access; install via `npm install github:isaacriehm/harness#<sha>` |
| Tarball (`pnpm pack`) | When friend can't use npm git installs; produces `isaacriehm-harness-<version>.tgz` |
| Direct repo clone + symlink | For collaborative debugging — clone repo, `npm link`, point adopting project at the link |
| npm publish (later) | Once we have a stable v0; published as `@isaacriehm/harness` |

Init script supports all: `npx @isaacriehm/harness init` works with the npm name, `npx github:isaacriehm/harness init` works with the git URL, `npx /path/to/local/harness init` works with the local clone.

**Sensor:** run `npx @isaacriehm/harness init` on a fresh dummy repo; verify all directories present, all scaffolded files have provenance frontmatter, and a `harness watch` + `harness run` start cleanly.
...
- All `core/src/*` modules can be referenced as `WORKFLOW.md sample-project.pilot_module` candidates
...
- `npx @isaacriehm/harness init` runs cleanly on at least one other project (you pick a sample side project)
