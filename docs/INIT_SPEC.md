...
`harness init` is the front door of Cairn. It adopts any project in one pass with minimal operator friction. This doc specifies the full init flow, what it detects, what it writes, and the UX rules it follows.
...
```bash
npx @isaacriehm/harness init [repo-path]
...
5. **Cairn never overwrites existing files it didn't create**, except `AGENTS.md` (merges, doesn't overwrite) and `.harness/config/` (updates `<slug>:` block only, leaves rest intact on re-init).
...
This phase runs in full view. The operator watches it happen — they don't answer questions, they watch Cairn figure out their project.

```
  Scanning /path/with spaces/example-repo...
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx @isaacriehm/harness hook session-start"
          }
        ]
      }
    ]
...
- If present: append the mapper's suggested `agentsMdAppend` block (if any) under a `## Cairn` heading. Never modifies existing content.
...
```
Cairn re-init — changes detected:
