...
- `workflow.md` uses `<project_name>` placeholder that init/seed.ts substitutes. Verified no leftover `sample-project` references in templates (ARCHITECTURE forbids).
...
| `WORKFLOW_GUIDE.md` §11 | Hardcoded `sample-project:` example workflow.md block. Per S1 illustrative-only is fine; flag once for re-readers. | Add a "this is illustrative" comment at top of §11. |
...
| `QUESTIONS.md` M1 | Says `@isaac/harness`. Reality `@isaacriehm/harness*` (4 packages). | Update or close-out (questions are mostly answered). |
