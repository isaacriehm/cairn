...
Cairn becomes the **project maintainer**. After install + adoption, the operator just uses Claude Code normally and harness:
...
```json
{
  ...
  "repository": "https://github.com/isaacriehm/harness",
  "license": "MIT"
}
```
...
- **v0 → v1**: GitHub URL distribution.
  - User runs `/plugin marketplace add isaacriehm/harness` once
  - Then `/plugin install harness@isaacriehm-harness`
  - Tag `v0.1.0`, `v0.2.0`, … on each release; users pull via `/plugin update harness@isaacriehm-harness`
...
if ! command -v harness > /dev/null 2>&1; then
  ...
  echo "    /plugin install harness@isaacriehm-harness   (Claude Code)"
  echo "    npm install -g @isaacriehm/harness            (CLI)"
...
jobs:
  harness:
    ...
    steps:
      ...
      - run: npm install -g @isaacriehm/harness
