...
discord.js v14.25.x is the current line; requires Node 22.12+. Fits sample-project monorepo's Node baseline.
...
| Cairn queue offline | Bot replies: "Cairn is down. Message saved." | Stores message in dead-letter queue, retries on harness boot |
| Cairn internal error | Bot replies: "Cairn errored: <code>" with thread for stack trace | Full trace only in thread, not main channel |
...
|---|---|---|---|
| **`harness/` (top-level workspace)** | Clean separation; matches `phone-ai/` precedent; sample-project product code untouched | One more workspace | **Pick** |
...
| Cairn intake enqueue | <100ms | BullMQ in-process |
...
5. **Persistence model for harness state.** Use a sidecar SQLite, reuse sample-project core's Postgres, or run an in-memory store with periodic JSON dump? In-memory simplest; SQLite cheapest durable option; postgres reuse means harness now depends on core/ being up.
...
10. **Bot identity.** Single bot user `sample-project-harness-bot` or per-environment (`harness-dev`, `harness-prod`)? Per-environment is cleaner for log filtering but doubles bot token mgmt.
